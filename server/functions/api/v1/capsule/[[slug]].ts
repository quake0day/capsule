...
export const onRequestGet: PagesFunction<Env> = async ({ params, env }) => {
  ...
  if (blob) return await serveBlob(env, blob.addrSlug, blob.filePath);

  const filesSlug = splitFilesTail(slug);
  if (filesSlug !== null) return await serveFilesList(env, filesSlug);

  return await serveCapsule(env, slug);
};
...
async function serveCapsule(env: Env, slug: string): Promise<Response> {
...
async function serveFilesList(env: Env, slug: string): Promise<Response> {
  const addr = parseAddress(slug);
  if (!addr) {
...
async function serveBlob(env: Env, addrSlug: string, filePath: string): Promise<Response> {
